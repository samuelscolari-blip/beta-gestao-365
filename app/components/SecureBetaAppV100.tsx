...
function ensureHeader(
  ...
  let cell = row.querySelector<HTMLTableCellElement>(`th[data-v100="${marker}"]`);
  if (!cell) {
    cell = document.createElement("th");
    cell.dataset.v100 = marker;
...
function ensureDataCell(
  ...
  let cell = row.querySelector<HTMLTableCellElement>(`td[data-v100="${marker}"]`);
  if (!cell) {
...
function syncUnifiedMachinesScreen() {
  ...
  const originalHeaders = Array.from(headerRow.cells).filter(
    (cell) => !cell.hasAttribute("data-v100"),
  );
...
export default function SecureBetaAppV100(props: Props) {
  useLayoutEffect(() => {
    let frame = 0;
