...
async function employeeByCode(employeeCode) {
...
async function enroll(payload) {
  ...
  if (current && current.deviceTokenHash !== deviceTokenHash) {
    return json(
      {
        ok: false,
        error:
          "Este colaborador já possui um celular vinculado. Faça o teste no aparelho cadastrado.",
      },
      409,
    );
  }
...
async function punch(payload) {
  ...
  if (!employee || !employee.active) {
    return json(
      {
        ok: false,
        error: "O colaborador ainda não possui cadastro facial ativo.",
      },
      404,
    );
  }
  ...
  const referenceEmbedding = validEmbedding(JSON.parse(employee.embeddingJson));
  ...
  try {
    ...
    // Mantém o horário ISO quando o navegador enviar um fuso inválido.
  }
...
export async function GET(request) {
  try {
    ...
    const employee = await employeeByCode(employeeCode);
    if (!employee) return json({ ok: true, enrolled: false, events: [] });
