...
const wrapperPath = new URL(
  "../app/components/SecureBetaAppV100.tsx",
  import.meta.url,
);
const cssPath = new URL("../app/v100-unified-machines.css", import.meta.url);
const pagePath = new URL("../app/page.tsx", import.meta.url);
const layoutPath = new URL("../app/layout.tsx", import.meta.url);

test("V100 ativa a tela unificada de Máquinas preservando toda a cadeia", async () => {
  const wrapper = await readFile(wrapperPath, "utf8");
  const page = await readFile(pagePath, "utf8");

  assert.match(wrapper, /import SecureBetaAppV97/);
  assert.match(wrapper, /return <SecureBetaAppV97/);
  assert.match(page, /SecureBetaAppV100/);
...
test("a V100 é carregada depois do Manual do Sistema V99", async () => {
  const layout = await readFile(layoutPath, "utf8");

  assert.match(
    layout,
    /v99-system-manual\.css";\nimport "\.\/v100-unified-machines\.css";/,
  );
});
